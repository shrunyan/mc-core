...
module.exports = class Validator {
  ...
  validate() {
    return new Promise((resolve, reject) => {
      logger.debug('Validator:validate', this)

      const VARS = []

      // Store these on instance so they can be invoked
      // in other class methods
      this.resolve = resolve
      this.reject = reject

      // Ensure provided variables align
      // with pipeline inputs
      this.validateInputs()

      // Merge user provided variable values
      // with pipeline defaults
      this.combine(this.variables, this.inputs)
  ...
  validateInputs() {
    // logger.debug('validator | validate variables map to inputs')
    //  // Convert the variable objects into a list of expected input/variable keys
    // let varKeys = this.variables.map(variable => variable.name)
  ...
  combine() {
    // TODO combine variable values with input defaults
  }
}
